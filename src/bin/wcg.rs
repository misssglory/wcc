...
impl Application {
    ...
    fn format_report(&self, graph: &CodeGraph, root: &Path) -> String {
        ...
        report.push_str(&format!("📊 Code Graph Analysis: {}\n", root.display()));
        report.push_str(&format!("{}\n", "=".repeat(80)));
        report.push_str(&format!("\n📈 Summary:\n"));
        report.push_str(&format!("  Structs: {}\n", graph.structs.len()));
        report.push_str(&format!("  Functions: {}\n", graph.functions.len()));
        report.push_str(&format!("  Implementations: {}\n", graph.impls.len()));
        ...
        if !graph.structs.is_empty() {
            ...
            for struct_info in graph.structs.values() {
                ...
                report.push_str(&format!(" ({})", rel_path.display()));
                report.push('\n');
                if self.show_fields && !struct_info.fields.is_empty() {
                    ...
                    for field in &struct_info.fields {
                        ...
                        if !field.attributes.is_empty() {
                            report.push_str(&format!(" [{}]", field.attributes.join(", ")));
                        }
                        report.push('\n');
                    }
                }
