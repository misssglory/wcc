...
impl CodeBlockInfo {
    ...
    fn format_header(&self, config: &UnifiedConfig) -> Result<String> {
    ...
    fn to_clipboard_payload(&self, config: &UnifiedConfig) -> Result<String> {
        let header = self.format_header(config)?;
    ...
    fn find_blocks_in_file(&self, file_path: &Path) -> Result<Vec<CodeBlockInfo>> {
        ...
        while i < lines.len() {
            ...
            if trimmed.starts_with("impl ") {
                eprintln!("Debug: Found impl block at line {}", i);
                if let Some(impl_info) = self.extract_impl_block(
                    ...
                    eprintln!(
                        "Debug: Extracted impl: {} (lines {}-{})",
                        impl_info.block_name, impl_info.start_line, impl_info.end_line
                    );
                    ...
                    while j < impl_end_line {
                        ...
                        if inner_trimmed.contains("fn ") && !inner_trimmed.starts_with("//") {
                            eprintln!(
                                "Debug: Found function inside impl at line {}: {}",
                                j, inner_trimmed
                            );
                            if let Some(func_info) = self.extract_function_block(
                                ...
                                if func_info.end_line <= impl_end_line {
                                    eprintln!(
                                        "Debug: Extracted function from impl: {}",
                                        func_info.block_name
                                    );
...
impl Application {
    ...
    fn print_block_summary(&self, blocks: &[CodeBlockInfo]) {
        ...
        eprintln!("\n\x1b[33mDebug - All found blocks:\x1b[0m");
        for block in blocks {
            match block.block_type {
                CodeBlockType::Function => eprintln!("  Function: {}", block.block_name),
                CodeBlockType::Impl => eprintln!("  Impl: {}", block.block_name),
                CodeBlockType::Struct => eprintln!("  Struct: {}", block.block_name),
            }
        }
    }
}
fn fn_name_contains(s: &str, pattern: &str) -> bool {
    s.contains(pattern)
        && !s.contains(&format!("{}_", pattern))
        && !s.contains(&format!("{}:", pattern))
}
fn main() -> Result<()> {
